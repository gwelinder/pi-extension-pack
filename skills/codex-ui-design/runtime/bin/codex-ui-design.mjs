...
Commands:
  ...
  imagegen --prompt TEXT | --prompts prompts.jsonl [--out DIR] [--concurrency N]
...
function buildInput({ text, images = [], imagegen = false, opts }) { const input = []; if (imagegen && opts.skillPath && existsSync(opts.skillPath)) input.push({ type: "skill", name: "imagegen", path: opts.skillPath }); input.push({ type: "text", text: imagegen ? `${text.trim()}\n\nUse the built-in image generation tool and produce exactly one raster image. Do not call shell scripts or external APIs. Do not ask for OPENAI_API_KEY.\n$imagegen` : text, text_elements: [] }); for (const img of images) input.push({ type: "localImage", path: path.resolve(img) }); return input; }
...
async function persistImage(run, opts) {
  ...
  const metadata = { id: run.job.id, prompt: run.job.prompt, source, sourcePath, finalPath, threadId: run.threadId, turnId: run.turnId, turnStatus: run.turnStatus, turnError: run.turnError, revisedPrompt: primary?.revisedPrompt || raw?.revised_prompt || null, imageStatus: primary?.status || raw?.status || null, messageText: run.messages.join("\n\n").slice(0, 4000), warnings: run.warnings, completedAt: new Date().toISOString() };
...
async function loadPromptJobs(opts) { const prompts = []; if (opts.prompt) prompts.push(...(Array.isArray(opts.prompt) ? opts.prompt : [opts.prompt])); if (opts.prompts) { const text = await fs.readFile(path.resolve(opts.prompts), "utf8"); prompts.push(...text.split(/\r?\n/).map(l => l.trim()).filter(Boolean)); } if (!prompts.length) throw new Error("provide --prompt TEXT or --prompts prompts.jsonl"); return prompts.map((line, idx) => { if (line.startsWith("{")) { const o = JSON.parse(line); return { id: o.id || `image-${idx+1}`, prompt: o.prompt || o.text || "" }; } return { id: `image-${idx+1}`, prompt: line }; }); }
...
async function main() {
  ...
  if (cmd === "imagegen") { const jobs = await loadPromptJobs(args); const { summaryPath } = await runImageJobs(jobs, opts); console.log(summaryPath); if (JSON.parse(readFileSync(summaryPath, "utf8")).failed) process.exitCode = 1; return; }
