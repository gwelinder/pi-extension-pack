...
function findCodeGraphProject(startPath: string): string | undefined {
  ...
  for (;;) {
    if (fs.existsSync(path.join(current, '.codegraph', 'codegraph.db')))
      return current;
...
function requireString(value: string | undefined, name: string): string {
  ...
  if (!trimmed)
    throw new Error(`codegraph ${name} is required for this action.`);
  return trimmed;
}
...
const ARG_BUILDERS: Record<CodeGraphAction, ArgsBuilder> = {
  ...
  files(params, projectPath) {
    ...
    if (params.maxDepth !== undefined)
      args.push('--max-depth', String(clampInt(params.maxDepth, 4, 1, 20)));
    if (
      params.format === 'flat' ||
  ...
  affected(params, projectPath) {
    const files = Array.isArray(params.files)
      ? params.files.filter((file) => file.trim())
      : [];
...
function mcpOnlyArgs(params: CodeGraphParams): string[] {
  throw new Error(
    `codegraph ${params.action} is only available through the MCP bridge.`
  );
}
...
function commandErrorMessage(error: unknown): string {
  ...
  const parts = [
    error.message,
    extra.stdout?.toString(),
    extra.stderr?.toString(),
  ]
    .filter(Boolean)
...
function handleMcpMessage(message: JsonRpcMessage, state: McpRunState): void {
  if (message.id === 1) {
    if (message.error) {
      state.finish(
        new Error(
          `CodeGraph MCP initialize failed: ${message.error.message || JSON.stringify(message.error)}`
        )
      );
      return;
    }
    ...
    state.send({
      ...
      id: 2,
      method: 'tools/call',
      params: { name: state.toolName, arguments: state.input },
    });
    return;
  }
  ...
  if (message.error) {
    state.finish(
      new Error(
        `CodeGraph MCP ${state.toolName} failed: ${message.error.message || JSON.stringify(message.error)}`
      )
    );
    return;
  }
  ...
  const isError = Boolean(
    (message.result as { isError?: boolean } | undefined)?.isError
  );
...
function runCodeGraphMcpTool(
  ...
  return new Promise((resolve, reject) => {
    ...
    timer = setTimeout(() => {
      finish(
        new Error(
          `CodeGraph MCP ${toolName} timed out after ${DEFAULT_TIMEOUT_MS}ms.${stderr ? `\n${stripAnsi(stderr).trim()}` : ''}`
        )
      );
    }, DEFAULT_TIMEOUT_MS);
    ...
    child.on('close', (code) => {
      if (!settled)
        finish(
          new Error(
            `CodeGraph MCP ${toolName} exited before returning a result (code ${code}).${stderr ? `\n${stripAnsi(stderr).trim()}` : ''}`
          )
        );
    });
...
function writeArtifact(
  ...
  const artifactPath = path.join(
    dir,
    `${stamp}_${action}_${sha256.slice(0, 12)}.txt`
  );
...
function shouldAutoSync(params: CodeGraphParams): boolean {
  return (
    params.sync !== false &&
    !['status', 'sync', 'init', 'index'].includes(params.action)
  );
}
...
const codegraphTool = defineTool({
  ...
  promptSnippet:
    'Use codegraph for indexed semantic code exploration before broad grep/read when a repo has a .codegraph index.',
  promptGuidelines: [
  ...
  parameters: Type.Object({
    ...
    query: Type.Optional(
      Type.String({
        description: 'Task/search text for context or search actions.',
      })
    ),
    symbol: Type.Optional(
      Type.String({
        description: 'Symbol name for callers, callees, impact, or node.',
      })
    ),
    from: Type.Optional(
      Type.String({ description: 'For trace: symbol where the flow starts.' })
    ),
    to: Type.Optional(
      Type.String({ description: 'For trace: symbol the flow should reach.' })
    ),
    files: Type.Optional(
      Type.Array(Type.String(), {
        description: 'Changed files for affected-test analysis.',
      })
    ),
    projectPath: Type.Optional(
      Type.String({
        description:
          'Project path override. Defaults to nearest .codegraph project or the current working directory.',
      })
    ),
    limit: Type.Optional(
      Type.Number({ description: 'Result limit for search/callers/callees.' })
    ),
    kind: Type.Optional(
      Type.String({
        description:
          'Optional search kind filter, e.g. function, class, interface, file, route.',
      })
    ),
    depth: Type.Optional(
    ...
    maxNodes: Type.Optional(
      Type.Number({ description: 'Max nodes for context (default 20).' })
    ),
    maxCode: Type.Optional(
      Type.Number({ description: 'Max code blocks for context (default 8).' })
    ),
    maxFiles: Type.Optional(
      Type.Number({
        description:
          'For explore: maximum files to include source from (default 12).',
      })
    ),
    maxDepth: Type.Optional(
      Type.Number({ description: 'Max tree depth for files action.' })
    ),
    filter: Type.Optional(
      Type.String({
        description: 'Directory filter for files, or test glob for affected.',
      })
    ),
    ...
    includeCode: Type.Optional(
      Type.Boolean({
        description: 'For context: include code snippets (default true).',
      })
    ),
    sync: Type.Optional(
      Type.Boolean({
        description: 'Auto-sync before query actions (default true).',
      })
    ),
    maxOutputChars: Type.Optional(
      Type.Number({
        description:
          'Max returned chars before artifact-backed truncation (default 24000).',
      })
    ),
    force: Type.Optional(
      Type.Boolean({ description: 'For index: force a full re-index.' })
    ),
    index: Type.Optional(
      Type.Boolean({
        description:
          'For init: run initial indexing after initialization (default true).',
      })
    ),
    quiet: Type.Optional(
      Type.Boolean({
        description:
          'For sync/index/affected: reduce CLI decoration where supported.',
      })
    ),
  }),
  async execute(_toolCallId, params: CodeGraphParams, signal, _onUpdate, ctx) {
    ...
    if (
      requiresExistingIndex(action) &&
      !fs.existsSync(path.join(projectPath, '.codegraph', 'codegraph.db'))
    ) {
      return {
        isError: true,
        content: [
    ...
    try {
      ...
      const artifact = truncation.truncated
        ? writeArtifact(ctx, action, fullOutput)
        : undefined;
      ...
      const suffix = artifact
        ...
        : '';
      return {
        content: [{ type: 'text', text: `${truncation.text}${suffix}` }],
        details,
      };
    } catch (error) {
  ...
  renderCall(args: CodeGraphParams, theme) {
    ...
    return new Text(
      `${theme.fg('toolTitle', theme.bold('codegraph'))} ${theme.fg('accent', args.action)} ${theme.fg('muted', short)}`,
      0,
      0
    );
  },
  renderResult(result, { expanded, isPartial }, theme) {
    if (isPartial)
      return new Text(theme.fg('muted', 'Querying CodeGraph…'), 0, 0);
    const details = result.details as Partial<CodeGraphDetails> | undefined;
    const content =
      result.content?.[0]?.type === 'text' ? result.content[0].text : '';
    const status = (result as { isError?: boolean }).isError
      ? theme.fg('error', 'failed')
      : theme.fg('success', 'ok');
    const summary = [
      `${theme.fg('toolTitle', theme.bold('CodeGraph'))} ${details?.action ?? ''} ${status} ${theme.fg('muted', `${details?.elapsedMs ?? 0}ms`)}`,
    ];
    if (details?.truncated && details.artifactPath)
      summary.push(theme.fg('warning', `full output: ${details.artifactPath}`));
    if (expanded && content)
      summary.push(
        '',
        ...content
