...
### Step 2: Map the folder, then read the evidence that matters
- Start with bounded reconnaissance: inventory direct children, context files, manifests, key entrypoints, and nearby docs/tests.
- Read existing `AGENTS.md`/`CLAUDE.md`, package/config files, and the key source files that establish the folder's actual conventions.
- For large projects, sample representative modules and follow cited dependencies instead of dumping every file into context.
- Read candidate source files fully before deriving a rule from them. Do not claim comprehensive coverage unless it was actually necessary and completed.
...
### Step 6: Reconcile AGENTS.md safely
First inspect whether `AGENTS.md` already exists.

- If absent, create a relative symlink: `ln -s CLAUDE.md AGENTS.md`.
- If it is already the correct symlink, leave it alone.
- If it is a real file or points elsewhere, do not overwrite it. Compare its instructions and ask before consolidating.
