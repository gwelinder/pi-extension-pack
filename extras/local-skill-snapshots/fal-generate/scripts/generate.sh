...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --model, -m       Model ID (default: fal-ai/nano-banana-2)" >&2
            echo "  --image-url       Input image URL for I2V models" >&2
            echo "  --file, --image   Local file (auto-uploads to fal CDN)" >&2
            echo "  --size            square, portrait, landscape (flux models)" >&2
            echo "  --resolution, -r  0.5K, 1K, 2K, 4K (nano-banana, default: 1K)" >&2
            echo "  --aspect-ratio, -a 16:9, 3:2, 1:1, etc (nano-banana, default: 16:9)" >&2
            echo "  --output-format, -f jpeg, png, webp (nano-banana, default: jpeg)" >&2
            ...
            echo "" >&2
            echo "Examples:" >&2
            echo "  # Generate 4K image with nano-banana-2 (default model)" >&2
            echo "  ./generate.sh -p \"a sunset\" -r 4K -a 16:9" >&2
            echo "" >&2
            echo "  # Generate with flux" >&2
            echo "  ./generate.sh -p \"a sunset\" -m fal-ai/flux/dev --size landscape" >&2
            echo "" >&2
            echo "  # Generate video async (returns immediately)" >&2
            echo "  ./generate.sh -p \"ocean waves\" -m fal-ai/veo3.1 --async" >&2
            echo "" >&2
            echo "  # Check status" >&2
...
EOF
    ...
    NB_ASPECT="${ASPECT_RATIO:-16:9}"
    NB_RES="${RESOLUTION:-1K}"
    NB_FMT="${OUTPUT_FORMAT:-jpeg}"
    PAYLOAD=$(python3 -c "
import json
d = {'prompt': '''$(echo "$PROMPT" | sed "s/'/'\\\\''/g")''', 'num_images': $NUM_IMAGES, 'aspect_ratio': '$NB_ASPECT', 'resolution': '$NB_RES', 'output_format': '$NB_FMT'}
print(json.dumps(d))
")
else
    # Flux and other standard T2I models
    EXTRA_FIELDS=""
    if [ -n "$ASPECT_RATIO" ]; then
        EXTRA_FIELDS=", \"aspect_ratio\": \"$ASPECT_RATIO\""
    fi
    if [ -n "$RESOLUTION" ]; then
        EXTRA_FIELDS="$EXTRA_FIELDS, \"resolution\": \"$RESOLUTION\""
    fi
    PAYLOAD=$(cat <<EOF
{"prompt": "$PROMPT", "image_size": "$IMAGE_SIZE", "num_images": $NUM_IMAGES$EXTRA_FIELDS}
EOF
)
fi
