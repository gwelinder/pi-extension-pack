...
**Level 1 — Discovery (often tens to hundreds of tokens per visible skill):**
Clients commonly inject `name` + `description` at startup. This cost compounds: a large catalog can consume thousands of tokens and dilute routing attention. Prefer a small visible core plus on-demand catalog search when the harness supports it.
...
- `name` is lowercase, hyphens only, 1–64 chars, and should match the parent folder under the Agent Skills standard. Pi is intentionally lenient about folder/name mismatch, but portable skills should still match.
...
4. **Write the smallest body that works.** Add only when testing reveals repeated, causal gaps.
5. **Replace before appending.** Remove obsolete guidance and keep maintenance diffs size-neutral or smaller unless they add a tested capability.
6. **Move detail to references/ once SKILL.md grows too long.**
7. **Test triggering.** Ask the agent something the skill should handle without invoking it explicitly. If it doesn't fire, fix the description or gateway metadata.
8. **Test execution.** Invoke explicitly. If output is wrong, fix the body.
9. **Adversarial test.** Have another LLM ask: "What edge cases break this skill?" Patch only validated gaps.
10. **Version control.** Treat skills as code. Tag, branch, review.
